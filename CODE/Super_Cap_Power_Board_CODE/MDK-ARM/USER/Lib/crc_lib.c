...
unsigned char turn_up(unsigned char input){
    unsigned char res = 0;
    for(int i = 7; i >= 0; i--){
        res += ((input&(0x01<<i))>>i)<<(7-i);
    }
    return res;
}
...
unsigned char CRC_GetCRC8CheckSum(unsigned char *pchMessage,  unsigned int dwLength,  char ucCRC8) {
    ...
    while (dwLength--) {
        ucIndex = ucCRC8 ^ turn_up(*pchMessage++);
        ucCRC8 = CRC8_TAB[ucIndex];
    }
    return turn_up(ucCRC8);
}

unsigned char CRC_GetCRC8CheckSum_2(unsigned char *pchMessage,  unsigned int dwLength,  char ucCRC8) {
    unsigned char ucIndex = 0;
    while (dwLength--) {
...
void CRC_AppendCRC8CheckSum(unsigned char *pchMessage,  unsigned int dwLength) {
    ...
    ucCRC = CRC_GetCRC8CheckSum_2((unsigned char *)pchMessage,  dwLength - 1,  CRC8_INIT);
