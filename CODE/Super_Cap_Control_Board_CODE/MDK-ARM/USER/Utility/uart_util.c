...
void Uart_RxIdCallback(UART_HandleTypeDef *huart)
{
	if (huart == Const_BTlog_UART_HANDLER)
		...
		{
				if((24 - Uart_DMACurrentDataCounter(huart->hdmarx->Instance)) == 8) 
				{
					//Boost_CommunicatorDecoder(Uart3_Board_Community_Buffer);
				}
							huart->hdmarx->Instance->CNDTR = 24 ;
				//__HAL_DMA_SET_COUNTER(huart->hdmarx, 24);
		}
}
...
{
//	if(huart == &huart2) {
//		Communicator_toBoss(Uart2_Board_Community_Buffer);
//		HAL_UART_Receive_IT(&huart2,Uart2_Board_Community_Buffer,8);
//	}
	if(huart == &huart3) {
		counter++;
		Cap_Communicator_toBoss(Uart3_Board_Community_Buffer);
